...
| E | 对话式讲座写操作：创建/修改/取消/发布四类草稿确认；删除旧 `/api/ai/*` 并统一到 `/api/agent/chat` | ✅ 已完成 |
| F | 教室推荐（按时间冲突过滤 + 容量贴合排序）与容量估算明细 | ✅ 已完成 |
| G | 通用主动任务：幂等键、优先级、DB 轮询调度、退避重试、卡死回收，含报表生成与提醒推送执行器 | ✅ 已完成 |
| H | 持久化与可观测（上线前必做）：agent_session / agent_message / agent_trace 落表；执行追踪查询接口 | 待办 |
| I | 可选进阶：SSE 流式输出；RAG 知识库答疑；学校画像文档上传（PDF/Word/MD） | 可选 |
