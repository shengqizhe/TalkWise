...
**新特性**
- 学生端"我的预约"卡片补齐操作：**签到**（已确认且未签到时显示，签到后显示"✓ 已签到"徽标）；**评价 / 修改评价**（讲座结束后可评，评分 + 文字，支持修改，500 字上限）
- 评价对话框：评分（el-rate 五档带文案）+ 评价内容（可选）
- 教师端"历史记录"页新增"评价反馈"入口（带讲座预选直达 `?lectureId=`）；评价反馈页支持参数预选

**背景**
- UI 重构时这两个入口丢失（后端接口一直在），导致"学生无法签到/写评价 → 评价分析没有数据源"
- 本次补齐后闭环打通：学生签到/评价 → 教师查看评价 → AI 评价分析报告
